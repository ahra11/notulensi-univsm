...
const UserManagement: React.FC<UserManagementProps> = ({ onNavigate }) => {
    ...
    // FITUR: Tambah User Manual (Cloud Sync)
    ...
    // FITUR: Reset Password & Salin Otomatis
        ...
        <div className="p-4 md:p-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-sm text-slate-500 font-medium italic">Sinkronisasi Cloud Aktif • Admin: {currentUser.name}</p>
                </div>
            ...
            {showAddForm && (
                <div className="mb-8 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xl animate-in zoom-in-95 duration-300">
                    <form onSubmit={handleAddUser} className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <input required type="email" placeholder="Email (Civitas)" value={newUser.email} onChange={e => setNewUser({...newUser, email: e.target.value})} className="p-3 bg-slate-50 border-none rounded-xl text-sm" />
                        ...
                        <input type="text" placeholder="NIP" value={newUser.nip} onChange={e => setNewUser({...newUser, nip: e.target.value})} className="p-3 bg-slate-50 border-none rounded-xl text-sm" />
                        ...
                        <button type="submit" className="bg-[#252859] text-white rounded-xl font-bold uppercase text-[10px] tracking-widest hover:bg-black transition-all">Simpan ke Sheets</button>
                    </form>
                </div>
            )}
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-slate-50/50 border-b border-slate-100 text-[10px] uppercase font-bold text-slate-400 tracking-widest">
                            <th className="px-6 py-4">Civitas (Klik Nama/NIP untuk Edit)</th>
