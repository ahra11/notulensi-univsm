...
const History: React.FC<HistoryProps> = ({ onNavigate }) => {
    ...
    // FITUR HAPUS KHUSUS PIMPINAN / SUPER ADMIN
    const handleDelete = async (e: React.MouseEvent, id: string) => {
        e.stopPropagation(); // Mencegah baris ikut terklik
        if (window.confirm("Peringatan: Anda akan menghapus dokumen ini secara permanen dari server. Lanjutkan?")) {
            ...
            try {
                ...
                alert("Gagal menghapus dokumen dari Cloud.");
            } finally {
                setIsLoading(false);
            }
        }
                                ...
                                <tr key={meeting.id} className="hover:bg-slate-50/50 transition-colors group">
                                    ...
                                    <td className="p-5">
                                        <div className="flex items-center justify-end gap-2">
                                            {/* TOMBOL EDIT & HAPUS KHUSUS PIMPINAN */}
                                            {(currentUser.role === 'PIMPINAN' || currentUser.role === 'SUPER_ADMIN') && (
                                                <>
                                                    <button onClick={(e) => { e.stopPropagation(); onNavigate('form', meeting); }} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all" title="Edit Dokumen">
                                            ...
                                            <button onClick={() => onNavigate('detail', meeting)} className="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 text-[#252859] hover:bg-[#252859] hover:text-white rounded-xl text-xs font-bold transition-all shadow-sm">
                                                Buka
