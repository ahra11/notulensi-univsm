...
const MinutesDetail: React.FC<MinutesDetailProps> = ({ minute, onNavigate }) => {
    ...
    // Gunakan logo grayscale untuk kesan hitam putih yang Anda minta
    // Anda bisa mengganti URL ini dengan URL raw dari GitHub Anda: 
    // misal: https://raw.githubusercontent.com/username/repo/main/Gambar%20Logo%20hitam%20putih%20Univsm.png
    ...
    return (
        <div className="min-h-screen bg-slate-100/50 print:bg-white pb-24 print:pb-0 font-serif relative">
            ...
            {/* Elegant Floating Action Bar - Discrete & High-End */}
            ...
            <div className="max-w-[210mm] mx-auto bg-white shadow-2xl md:my-16 p-[15mm] md:p-[20mm] print:shadow-none print:m-0 print:p-0">
                ...
                {/* Metadata Formal - Khusus Agenda Staf/Dosen */}
                <div className="grid grid-cols-1 gap-y-1.5 mb-12 text-[10.5pt]">
                    ...
                    <div className="flex items-start">
                        <span className="font-bold w-44 shrink-0">Status Dokumen</span>
                        <span className="w-4 shrink-0">:</span>
                        <span className="font-bold uppercase tracking-widest">{minute.status}</span>
                    </div>
                    <div className="flex items-start">
                        <span className="font-bold w-44 shrink-0">Notulis (Penyusun)</span>
                        <span className="w-4 shrink-0">:</span>
                        <span className="font-bold uppercase">{minute.submittedBy}</span>
                    </div>
                </div>

                {/* Isi Pembahasan - Font Lebih Besar & Rapi */}
                <div className="min-h-[500px] border-t border-slate-100 pt-8">
                    ...
                    <div className="space-y-6 text-[11.5pt] leading-relaxed text-black text-justify">
                        {discussionPoints.length > 0 ? (
                            ...
                            <p className="italic text-slate-400">Data pembahasan tidak ditemukan atau belum diinput.</p>
                        )}
                    </div>
                </div>

                {/* Tanda Tangan - Format Klasik */}
                <div className="mt-28 grid grid-cols-2 gap-32 text-[10.5pt]">
                    <div className="text-center">
                        ...
                        <div className="border-t-[1.2pt] border-black pt-2 inline-block min-w-[240px]">
                            ...
                            <p className="text-[8pt] uppercase mt-0.5">Pimpinan / Pejabat Berwenang</p>
                        </div>
                    </div>
                    <div className="text-center">
                        ...
                        <div className="border-t-[1.2pt] border-black pt-2 inline-block min-w-[240px]">
                            ...
                            <p className="text-[8pt] uppercase mt-0.5">Staf / Dosen Terkait</p>
                        </div>
                    </div>
                </div>
                ...
                {minute.documentation && minute.documentation.length > 0 && (
                    <div className="mt-24 pt-12 border-t-[0.5pt] border-slate-300 print:page-break-before-always">
                        <h4 className="text-[11pt] font-bold uppercase tracking-[0.2em] mb-10 text-center underline underline-offset-8">LAMPIRAN DOKUMENTASI</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            {minute.documentation.map((img, idx) => (
                                <div key={idx} className="border-[0.8pt] border-black p-2 bg-white shadow-sm">
                                    <img src={img} alt={`Dokumentasi ${idx + 1}`} className="w-full h-auto grayscale" />
                                    <p className="text-[8pt] text-center mt-2 italic">Gambar {idx + 1}: Lampiran Rapat</p>
                                </div>
                            ))}
                        </div>
                    </div>
